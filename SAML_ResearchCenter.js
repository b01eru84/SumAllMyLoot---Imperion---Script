...
function showOnResearchCenter(){
	...
	if (isResearchCenter)
	{
		...
		for(var i = 1; i < researchTables.length; i++)
		{
			...
			var extraTableDiv = researchCenterMainDiv.getElementsByClassName('extraTable')[i];		
			...
			{	
				try
				{
					...
					var days = showDays(((parseFloat(requiredPoints) - parseFloat(currPoints[i])) / parseFloat(productionPoints[i])).toFixed(2));
